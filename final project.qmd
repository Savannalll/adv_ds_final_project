...
# Define the Elastic Net Regression fitting function with cross-validation
fit_elastic_net <- function(data, penalty, alpha) {
  
  # Define the recipe
  elastic_net_rec <- recipe(sub_pro ~ ., data = final_df) %>%
    add_role(fipsplace_00, year, new_role = "id") %>%
    step_rm(has_role("id")) %>%
    step_normalize(all_predictors()) %>%
    step_impute_mean(all_predictors())

  # Define the model specification for Elastic Net
  elastic_net_mod <- linear_reg(penalty = penalty, mixture = alpha) %>%
    set_mode("regression") %>%
    set_engine("glmnet")

  # Create the workflow
  elastic_net_wf <- workflow() %>%
    add_recipe(elastic_net_rec) %>%
    add_model(elastic_net_mod)

  # Perform 10-fold cross-validation
  vfold_res <- vfold_cv(data, v = 10)
  fit_res <- fit_resamples(elastic_net_wf, resamples = vfold_res)

  # Extract and return the performance metric 
  perf <- collect_metrics(fit_res)
  rmse_val <- perf %>% filter(.metric == "rmse") %>% summarise(mean_rmse = mean(mean)) %>% pull(mean_rmse)
  return(data.frame(penalty = penalty, alpha = alpha, rmse = rmse_val))
}

penalty_values <- seq(0, 1, length.out = 10)
alpha_values <- seq(0, 1, length.out = 5)

# Perform grid search 
grid <- expand.grid(penalty = penalty_values, alpha = alpha_values)
grid_results <- map_dfr(grid, ~fit_elastic_net(final_df, .x$penalty, .x$alpha))

# Plot the results
ggplot(grid_results, aes(x = penalty, y = rmse, color = as.factor(alpha))) +
  geom_line() +
  labs(x = "Penalty", y = "RMSE", color = "Alpha", title = "Elastic Net Regression RMSE vs. Penalty and Alpha")
